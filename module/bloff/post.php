...
$comments = mysqli_fetch_array(mysqli_query($COMMENTBD, "SELECT * FROM `bloff".$page."`"));
if (!$comments) {
  $sql = "CREATE TABLE `bloff".$page."` ( `id` INT NOT NULL AUTO_INCREMENT , `mainid` INT(255) NOT NULL , `user` INT(255) NOT NULL , `text` TEXT NOT NULL , `date` DATE NOT NULL , `time` TIME NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;";
...
$commentsNumber = mysqli_fetch_array(mysqli_query($COMMENTBD , "SELECT COUNT(*) FROM `bloff".$page."`"));
$i = $commentsNumber[0];
$MainCommentsNumber = mysqli_fetch_array(mysqli_query($COMMENTBD , "SELECT COUNT(*) FROM `bloff".$page."` WHERE `mainid`=0 "));

echo '<center>Комментарии('.$MainCommentsNumber[0].')</center>';
echo '<div style="display:inline-block;width:100%;">';
while ($i >= 1) {
$comment = mysqli_fetch_array(mysqli_query($COMMENTBD, "SELECT * FROM `bloff".$page."` WHERE `id` = '".$i."'"));
if ($comment['mainid']=="0") {
...
<br><br>
<form class="" action="/bloff/query/commentadd" method="post">
  <input type="hidden" name="id" value="<?php echo $page; ?>" >
    <input type="hidden" id="mainid" name="mainid" value="0" >
  <input type="hidden" name="tablename" value="<?php echo "bloff".$page; ?>" >
